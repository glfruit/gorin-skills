...
def _advance_bilingual_stage(download_manifest: Path) -> int:
    ...
    reviewed_record = (
        subtitle.get("reviewed_source_srt") if isinstance(subtitle, dict) else None
    )
    reviewed_value = (
        reviewed_record.get("path") if isinstance(reviewed_record, dict) else None
    )
    ...
    if (
        deliverable == "full"
        and kind == "automatic"
        and execution.get("asr_review_requested") is True
        and not isinstance(reviewed_value, str)
    ):
        media_record = artifacts.get("intermediate") if isinstance(artifacts, dict) else None
        media_value = media_record.get("path") if isinstance(media_record, dict) else None
        media_path = None
        if isinstance(media_value, str):
            candidate = Path(media_value)
            media_path = candidate if candidate.is_absolute() else output_dir / candidate
        platform_path = Path(source_value)
        if not platform_path.is_absolute():
            platform_path = output_dir / platform_path
        manifest["status"] = "asr_transcription_required"
        execution.update(
            {
                "complete": False,
                "next_stage": "asr_transcription_required",
                "platform_source_srt": str(platform_path),
                "asr_media": str(media_path) if media_path is not None else None,
            ...
            json.dumps(
                {
                    ...
                    "source_language": language,
                    "instruction": (
                        "Create an explicitly approved MLX Whisper hypothesis, "
                        "adjudicate only the generated disagreement batches, then "
                        "apply the validated reviewed source to this manifest."
                    ),
                },
                ensure_ascii=False,
                sort_keys=True,
    ...
    reviewed_source_is_authorized = (
        kind == "automatic"
        and execution.get("asr_review_requested") is True
        and isinstance(reviewed_value, str)
    )
    selected_source_value = reviewed_value if reviewed_source_is_authorized else source_value
    source_srt = Path(selected_source_value)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Pause automatic-caption delivery for optional ASR review before "
            "translation; model installation and downloads are separate actions"
        ),
    )
