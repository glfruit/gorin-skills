...
8. Keep context small: never read the full subtitle/review manifest, all batches at once, or raw FFmpeg logs.
...
When the user explicitly requests higher-confidence review of platform
automatic captions for a `full` delivery, add `--review-auto-subs`. This opts
into a later review state; it does not authorize installing MLX or downloading
model weights.
...
### Exit 3: automatic-caption ASR review required

If the returned status is `asr_transcription_required`, read
[asr-review-contract.md](references/asr-review-contract.md). This state occurs
only when `--review-auto-subs` was explicitly requested and the selected track
is automatic. Manual tracks bypass it.

Run `mlx_whisper_adapter.py --dry-run` first with the exact Python interpreter
from the approved isolated MLX installation; do not assume the default
`python3` shares that environment. Installing `mlx-whisper` and downloading
remote model weights are separate approval-required host changes; prefer an
already approved, pinned local model directory. After transcription,
request and adjudicate one compact disagreement batch at a time with
`asr_review.py next-batch`, then `render` and `apply`. The platform subtitle
remains immutable; Whisper is only a second hypothesis. A successful `apply`
returns Exit 3 again with `bilingual_required`.
...
- Require `mlx-whisper` only for an explicitly requested automatic-caption review. Its adapter refuses an unapproved remote model download and records the selected model and media hash; do not treat its output as authoritative.
