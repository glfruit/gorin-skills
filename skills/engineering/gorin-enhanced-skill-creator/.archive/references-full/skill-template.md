...
## Security & Portability (Non-Negotiable)

### No Hardcoded Paths or Secrets
- User-specific paths (`/Users/...`, `/home/.../username`) are **never** hardcoded
- Use environment variables with `os.environ.get("VAR", default_value)` pattern in Python
- Use `${VAR:-default}` pattern in shell wrappers
- API keys, tokens, passwords only in environment variables — never in source code
- Repo URLs read from `GORIN_SKILLS_REPO` env var, never hardcoded
- Document required env vars in `.skill-meta.json` → `env_vars` array

### `.skill-meta.json` (Required)

Place at skill root. Travels with the skill in git, canonical source of origin truth.

```json
{
  "origin": "self | third-party",
  "author": "gorin | external",
  "version": "1.0",
  "description": "One-line description of what the skill does",
  "dependencies": {
    "runtime": "Python 3.12+ | bash | node",
    "python_packages": ["package1", "package2"],
    "external": ["optional CLI tools"]
  },
  "env_vars": ["REQUIRED_VAR_1", "OPTIONAL_VAR_2"]
}
```

For third-party or ClawHub-installed skills, `"origin": "third-party", "author": "external"`.
