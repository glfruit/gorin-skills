...
def load_outline(excel_path: str, template_type: str | None = None) -> list[OutlineItem]:
    ...
    missing_headers = [header for header in EXPECTED_HEADERS if header not in column_map]
    if missing_headers:
        raise ValueError(
            "missing required outline headers: " + ", ".join(missing_headers)
        )
    ...
    if not items:
        raise ValueError("outline contains no reviewable knowledge points")
    return items
...
def map_outline_items(items: list[OutlineItem], manuscript_text: str, headings: list[Heading], sections: dict[str, str]):
    ...
    for item in items:
        ...
        # Structural labels establish module presence, but they are not evidence
        # that a knowledge point or its required content is actually covered.
        search_targets = [item.point, item.summary]
