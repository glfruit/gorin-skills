...
Diagnose the exact yt-dlp executable that JZSub invokes. Its verbose diagnostics report configured `JS runtimes` and challenge providers. Package managers such as Nix may wrap yt-dlp with a private runtime that is unavailable in the parent shell's `PATH`, so `command -v deno` alone is not evidence that yt-dlp lacks Deno. Preserve runtime warnings emitted by yt-dlp itself; do not synthesize a warning or reject `--allow-remote-ejs` solely from the parent process environment.
