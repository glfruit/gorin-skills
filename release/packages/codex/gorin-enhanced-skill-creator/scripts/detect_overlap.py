#!/usr/bin/env python3
"""detect_overlap.py — Collision / overlap checker for skills."""
import json
import re
import sys
...
STOPWORDS = {
    'the', 'a', 'an', 'and', 'or', 'to', 'of', 'for', 'with', 'when', 'use', 'it',
    'this', 'that', 'is', 'are', 'be', 'as', 'on', 'in', 'at', 'by', 'from',
    'skill', 'skills', 'agent', 'agents', 'user', 'users', 'using', 'used', 'tool',
    'tools', 'task', 'tasks', 'request', 'requests', 'assistant',
    'do', 'not', 'only', 'into', 'than', 'then', 'their', 'them', 'your', 'you',
    'can', 'will', 'should', 'about', 'needs', 'want', 'wants', 'need',
}
GENERIC_TOKENS = {
    'create', 'build', 'make', 'help', 'update', 'manage', 'process', 'analyze',
    'generate', 'improve', 'workflow', 'content', 'real', 'patterns',
}
...
def main():
    if len(sys.argv) < 2:
        print('{"error": "Usage: detect_overlap.py <skill-directory> [skills-root]"}', file=sys.stderr)
        sys.exit(1)

    skill_dir = Path(sys.argv[1].rstrip("/"))
    script_dir = Path(__file__).resolve().parent
    default_skills_root = script_dir.parent.parent
    skills_root = Path(sys.argv[2].rstrip("/")) if len(sys.argv) > 2 else default_skills_root
