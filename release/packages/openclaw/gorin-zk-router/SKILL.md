---
name: gorin-zk-router
description: PKM 统一入口。zk <内容> 智能路由。pks/pkr/pko/pkp/pkh 快捷命令。
license: MIT
user-invocable: true
command-dispatch: tool
metadata: {"openclaw":{"requires":{"skills":["gorin-pkm-core","gorin-pkm-save-note","zk-literature","gorin-idea-creator","gorin-pkm-para-manager","obsidian-md"]}}}
---
...
类型→目录映射由 `gorin-pkm-core` 定义；配置路径使用 `PKM_VAULT_CONFIG`，默认 `~/.config/gorin-skills/pkm/vault-config.yaml`。
...
```bash
调用 `gorin-pkm-para-manager` 的 `scripts/health-v2.py --vault <vault-path>`。
```
