---
name: gorin-ai-builder-digest
description: 每日 AI 行业信息聚合 digest，覆盖中英文 builder/podcast/blog 来源。多源抓取→去重→中文摘要→发
  Telegram 群+存 PKM 笔记。当用户要求每日 AI 新闻聚合、AI builder 动态收集、创建/运行 AI digest 时触发。Don't
  use for single-article summaries (use web_reader+zk directly) or generic RSS
  monitoring (use blogwatcher instead) or non-AI content aggregation.
license: MIT
user-invocable: true
---
...
```bash
# 来源列表
cat "${GORIN_AI_BUILDER_CONFIG:-$HOME/.config/gorin-skills/gorin-ai-builder-digest/sources.json}"

# 状态文件（幂等关键）
cat "${GORIN_AI_BUILDER_STATE:-$HOME/.local/state/gorin-skills/gorin-ai-builder-digest/last-digest.json}" 2>/dev/null || echo "{}"
```

状态文件由 `GORIN_AI_BUILDER_STATE` 指定，默认位于用户 state 目录，不属于技能源码或目标包。
...
- **web_reader 脚本路径**：由 `GORIN_WEB_READER_SCRIPT` 显式提供；依赖的 canonical skill ID 为 `gorin-web-reader`。
...
- 检查 `GORIN_WEB_READER_SCRIPT` 指向的脚本是否存在
