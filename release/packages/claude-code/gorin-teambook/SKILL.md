---
name: gorin-teambook
description: Teambook multi-platform content aggregation system. Collects,
  processes, and delivers curated content feeds from multiple sources to
  Telegram channels. Do NOT use for general Telegram messaging or
  non-content-delivery tasks.
license: MIT
---
...
- 配置文件：`GORIN_TEAMBOOK_CONFIG`，默认 `~/.config/gorin-skills/gorin-teambook/config.json`
...
```bash
API_KEY=$(python3 -c "import json, os, pathlib; p=os.environ.get('GORIN_TEAMBOOK_CONFIG', pathlib.Path.home()/'.config/gorin-skills/gorin-teambook/config.json'); c=json.load(open(p)); print(c['agents']['{AGENT_NAME}']['api_key'])")
```
