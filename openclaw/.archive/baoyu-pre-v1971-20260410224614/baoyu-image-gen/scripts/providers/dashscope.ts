...
export async function generateImage(
  ...
  if (args.referenceImages.length > 0) {
    throw new Error(
      "Reference images are not supported with DashScope provider in baoyu-image-gen. Switch to a ref-capable provider such as --provider google (Gemini multimodal), --provider openai (GPT Image edits), or another provider that supports reference images."
    );
  }
