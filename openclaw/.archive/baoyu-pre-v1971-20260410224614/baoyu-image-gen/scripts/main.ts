...
function getConfigRoots(cwd: string, home: string): {
  ...
  return {
    projectConfigRoot: path.join(cwd, ".openclaw", "skills-config", "baoyu"),
    userConfigRoot: path.join(home, ".openclaw", "skills-config", "baoyu"),
    legacyProjectRoot: path.join(cwd, ".baoyu-skills"),
    legacyUserRoot: path.join(home, ".baoyu-skills"),
  };
}
...
Options:
  ...
  --provider google|openai|dashscope|replicate  Force provider (otherwise resolve from EXTEND.md, then env/available keys)
  -m, --model <id>          Model ID
...
Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.openclaw/skills-config/baoyu/.env > ~/.openclaw/skills-config/baoyu/.env (legacy .baoyu-skills supported as fallback).`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const roots = getConfigRoots(cwd, home);

  const envFiles = [
    path.join(roots.projectConfigRoot, ".env"),
    path.join(roots.userConfigRoot, ".env"),
    path.join(roots.legacyProjectRoot, ".env"),
    path.join(roots.legacyUserRoot, ".env"),
  ];
...
function tryParseRawJson(content: string): Partial<ExtendConfig> | null {
  try {
    const parsed = JSON.parse(content) as Partial<ExtendConfig>;
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(roots.projectConfigRoot, "baoyu-image-gen", "EXTEND.md"),
    path.join(roots.userConfigRoot, "baoyu-image-gen", "EXTEND.md"),
    path.join(roots.legacyProjectRoot, "baoyu-image-gen", "EXTEND.md"),
    path.join(roots.legacyUserRoot, "baoyu-image-gen", "EXTEND.md"),
  ];
...
function detectProvider(args: CliArgs): Provider {
  ...
  if (args.provider) {
    if (args.provider === "google" && !(process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY) && process.env.OPENAI_API_KEY) {
      throw new Error(
        "--provider google was requested, but GOOGLE_API_KEY/GEMINI_API_KEY is not configured. This environment does have OPENAI_API_KEY. If your EXTEND.md default_provider is openai or you are using an OpenAI-compatible proxy model (for example gemini-3-pro-image-preview via OPENAI_BASE_URL), remove --provider google and retry."
      );
    }
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or REPLICATE_API_TOKEN.\n" +
      "Create ~/.openclaw/skills-config/baoyu/.env or <cwd>/.openclaw/skills-config/baoyu/.env with your keys."
  );
}
...
async function main(): Promise<void> {
  ...
  console.error(`Using ${provider} / ${model}`);
  console.error(`Switch model: --model <id> | EXTEND.md default_model.${provider} | env ${provider.toUpperCase()}_IMAGE_MODEL`);
