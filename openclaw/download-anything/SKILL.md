...
- **B站需要浏览器 cookies 才能下载会员内容**：yt-dlp 默认只能下载免费视频。VIP/大会员内容需加 `--cookies-from-browser chrome` 或 `--cookies-from-browser safari`，依赖本地已登录的 B站账号。
- **aria2 高并发会触发服务器限流**：`-x 16 -s 16` 对小站点或 CDN 会返回 429/403。遇到时降低到 `-x 4 -s 4`，或加 `--max-connection-per-server=4`。
- **百度网盘搜索结果经常失效**：百度网盘分享链接随时可能被删除或过期。搜到资源后尽快下载，不要先收藏后下载。同时准备多个搜索源交叉验证。
- **gallery-dl 对某些网站需要特定 extractor 配置**：Twitter/X、Pixiv 等需要额外的 extractor 配置或 cookies。遇到 `Unsupported URL` 时查阅 gallery-dl 文档中对应站点的配置说明。
- **yt-dlp 格式选择不是自动的**：不加 `-f` 参数时 yt-dlp 会选默认格式（可能是低分辨率）。明确指定 `-f "bv*+ba/b"` 确保最佳画质。
